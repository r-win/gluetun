...
type API interface {
	FetchInfo(ctx context.Context, logger types.Logger, ip netip.Addr) (
...
const (
	...
	IPLocation  Provider = "iplocation"
...
func New(provider Provider, client *http.Client, token string) ( //nolint:ireturn
	...
	case IPLocation:
		return newIPLocation(client, token), nil
	case IP2Location:
...
func ParseProvider(s string) (provider Provider, err error) {
	...
	case "ipapi":
		return IPApi, nil
	default:
		return "", fmt.Errorf(`%w: %q can only be "ipinfo", "iplocation", "ip2location" or "ipapi"`,
